...
pub struct AgentPane {
    ...
    /// Unix timestamp of last state update (any field change).
    /// More reliable than status_ts for liveness -- updates on every persist.
...
pub enum BackendType {
    ...
    /// Zellij backend
    Zellij,
    /// nsmux backend (choam-io/cmux native macOS terminal)
    Nsmux,
}
...
impl std::fmt::Display for BackendType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            BackendType::Nsmux => write!(f, "nsmux"),
        }
    }
}
...
impl std::str::FromStr for BackendType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            ...
            "nsmux" | "cmux" => Ok(BackendType::Nsmux),
